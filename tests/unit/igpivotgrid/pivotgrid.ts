...
export function main() {
    describe('Infragistics Angular2 PivotGrid', () => {
		it('should initialize correctly', injectAsync([TestComponentBuilder, AsyncTestCompleter], (tcb: TestComponentBuilder, async) => {
           var template = '<div><ig-pivot-data-selector [widgetId]="selectorId" [options]="optsSelector"></ig-pivot-data-selector><ig-pivot-grid [widgetId]="gridId" [options]="optsGrid"></ig-pivot-grid></div>';
           tcb.overrideTemplate(TestComponent, template)
               .createAsync(TestComponent)
               .then((fixture) => {
					fixture.detectChanges();
					expect(fixture.debugElement.componentInstance.viewChild).toBeAnInstanceOf(Infragistics.IgPivotGridComponent);
					async.done();
               });
         }));
...
@Component({
    ...
    directives: [Infragistics.IgPivotGridComponent]
})
class TestComponent {
   private optsGrid: IgPivotGrid;
	...
	@ViewChild(Infragistics.IgPivotGridComponent) public viewChild: Infragistics.IgPivotGridComponent;
